...
public class ReferencePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HitroViewsRepository.Reference.Properties.entitytype)) {
				// init part
				entitytypeSettings = new EObjectFlatComboSettings(reference, HitroPackage.eINSTANCE.getReference_Entitytype());
			...
			if (isAccessible(HitroViewsRepository.Reference.Properties.linkType))
				basePart.setLinkType(EEFConverterUtil.convertToString(XMLTypePackage.Literals.IDREF, reference.getLinkType()));
			...
			if (isAccessible(HitroViewsRepository.Reference.Properties.entitytype)) {
				basePart.addFilterToEntitytype(new ViewerFilter() {
					...
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof String && element.equals("")) || (element instanceof Huientity); //$NON-NLS-1$ 
					}
					
				});
				// Start of user code for additional businessfilters for entitytype
				// End of user code
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HitroViewsRepository.Reference.Properties.linkType == event.getAffectedEditor()) {
			reference.setLinkType((java.lang.String)EEFConverterUtil.createFromString(XMLTypePackage.Literals.IDREF, (String)event.getNewValue()));
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (HitroPackage.eINSTANCE.getReference_Entitytype().equals(msg.getFeature()) && basePart != null && isAccessible(HitroViewsRepository.Reference.Properties.entitytype))
				basePart.setEntitytype((EObject)msg.getNewValue());
			if (HitroPackage.eINSTANCE.getReference_LinkType().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(HitroViewsRepository.Reference.Properties.linkType)) {
				if (msg.getNewValue() != null) {
					basePart.setLinkType(EcoreUtil.convertToString(XMLTypePackage.Literals.IDREF, msg.getNewValue()));
				} else {
			...
			try {
				if (HitroViewsRepository.Reference.Properties.linkType == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(HitroPackage.eINSTANCE.getReference_LinkType().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(HitroPackage.eINSTANCE.getReference_LinkType().getEAttributeType(), newValue);
				}
