...
public class DependsPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public DependsPropertiesEditionComponent(PropertiesEditingContext editingContext,
            EObject depends, String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(HitroViewsRepository.Depends.Properties.option)) {
                // init part
                optionSettings = new EObjectFlatComboSettings(depends,
            ...
            if (isAccessible(HitroViewsRepository.Depends.Properties.value))
                basePart.setValue(EEFConverterUtil.convertToString(XMLTypePackage.Literals.STRING,
                        depends.getValue()));
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (HitroViewsRepository.Depends.Properties.value == event.getAffectedEditor()) {
            depends.setValue((java.lang.String) EEFConverterUtil.createFromString(
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (HitroPackage.eINSTANCE.getDepends_Inverse().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(HitroViewsRepository.Depends.Properties.inverse))
                basePart.setInverse((Boolean) msg.getNewValue());

            if (HitroPackage.eINSTANCE.getDepends_Option().equals(msg.getFeature())
                    && basePart != null
                    && isAccessible(HitroViewsRepository.Depends.Properties.option))
                basePart.setOption((EObject) msg.getNewValue());
            if (HitroPackage.eINSTANCE.getDepends_Value().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(HitroViewsRepository.Depends.Properties.value)) {
                if (msg.getNewValue() != null) {
                    basePart.setValue(EcoreUtil.convertToString(XMLTypePackage.Literals.STRING,
                            msg.getNewValue()));
                } else {
                    basePart.setValue("");
                }
        ...
        if (event.getNewValue() != null) {
            try {
                if (HitroViewsRepository.Depends.Properties.inverse == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                HitroPackage.eINSTANCE.getDepends_Inverse().getEAttributeType(),
                                (String) newValue);
                    }
                ...
                if (HitroViewsRepository.Depends.Properties.value == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                HitroPackage.eINSTANCE.getDepends_Value().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE.validate(
                            HitroPackage.eINSTANCE.getDepends_Value().getEAttributeType(),
