...
public class ParamPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HitroViewsRepository.Param.Properties.id))
				basePart.setId(EEFConverterUtil.convertToString(XMLTypePackage.Literals.ID, param.getId()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HitroViewsRepository.Param.Properties.id == event.getAffectedEditor()) {
			param.setId((java.lang.String)EEFConverterUtil.createFromString(XMLTypePackage.Literals.ID, (String)event.getNewValue()));
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (HitroPackage.eINSTANCE.getParam_Mixed().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(HitroViewsRepository.Param.Properties.mixed)) {
			...
			if (HitroPackage.eINSTANCE.getParam_Id().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(HitroViewsRepository.Param.Properties.id)) {
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			HitroPackage.eINSTANCE.getParam_Mixed(),
			HitroPackage.eINSTANCE.getParam_Id()		);
			...
			try {
				if (HitroViewsRepository.Param.Properties.mixed == event.getAffectedEditor()) {
					...
					for (Iterator iterator = ((List)event.getNewValue()).iterator(); iterator.hasNext();) {
						chain.add(Diagnostician.INSTANCE.validate(HitroPackage.eINSTANCE.getParam_Mixed().getEAttributeType(), iterator.next()));
					}
					ret = chain;
				}
				if (HitroViewsRepository.Param.Properties.id == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(HitroPackage.eINSTANCE.getParam_Id().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(HitroPackage.eINSTANCE.getParam_Id().getEAttributeType(), newValue);
				}
